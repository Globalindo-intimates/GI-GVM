                                        ...
                                        <tr class="{{ $index % 1 == 0 ?: 'bg-gray-50' }} hover:bg-gray-100 transition-colors">
                                            ...
                                            <td class="border border-gray-300 px-3 py-2">
                                                <select name="{{ $fieldName }}"
                                                    ...
                                                    data-field="{{ $fieldName }}" data-item="{{ $item }}">
                                                    <option value="">-</option>
                                                    <option value="✔">✔</option>
                                                    <option value="✖">✖</option>
                                                </select>
                                                <!-- Hidden file input for damage image -->
                                                <input type="file" name="{{ $fieldName }}_image" id="{{ $fieldName }}_image"
                                                    class="hidden damage-image-input" accept="image/*">
                                                <!-- Hidden input for damage reason -->
                                                <input type="hidden" name="{{ $fieldName }}_reason" id="{{ $fieldName }}_reason"
                                                    class="damage-reason-input">
                                                <span id="{{ $fieldName }}_info"
                                                    class="text-xs text-green-600 block mt-1"></span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <div id="damageImageModal" class="fixed inset-0 hidden bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg max-w-md p-6 relative">
            ...
            <div class="mb-4">
                ...
                <textarea id="modalReasonInput" rows="3"
            ...
            <div class="mb-4">
                ...
                <input type="file" id="modalImageInput" accept="image/*" class="form-input border border-slate-200">
    ...
    <script>
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#no_polisi').on('change', function () {
                        ...
                        // Check existing data when vehicle changes
                        checkDuplicateData();
                    });
                }
            })
            ...
            function checkDuplicateData() {
                ...
                const id_kendaraan = $('#no_polisi').val();

                // Only check if both fields are filled
                if (tanggal && id_kendaraan) {
                    $.ajax({
                        url: '{{url('/check-existing-data')}}',
                        type: 'GET',
                        dataType: 'JSON',
                        data: {
                            tanggal: tanggal,
                        ...
                        success: function (response) {
                            if (response.exists) {
                                Swal.fire({
                                    ...
                                    text: 'Maintenance data for this vehicle on the selected date already exists. Please choose a different date.',
                                    confirmButtonColor: '#d33',
                                });
            ...
            // Check duplicate when date changes
            ...
            $('#mainForm').on('submit', function (e) {
                ...
                // Check for incomplete damage reports
                ...
                // Final check for duplicate before submit
                const tanggal = $('#tanggal').val();
                const id_kendaraan = $('#no_polisi').val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.exists) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Cannot Submit!',
                                text: 'Maintenance data for this vehicle on the selected date already exists.',
                                confirmButtonColor: '#d33',
                            });
                        } else {
                    ...
                    error: function (xhr, status, error) {
                        console.error('Error checking data:', error);
                        // If error checking, allow form to submit
                        $('#mainForm').off('submit').submit();
                    }
                });
            });
